#!/usr/bin/env python3
"""
Web Search Agent v2 - Searches for companies using Google Search API
and extracts contact information (name, email, address, phone)
Includes filtering for empty/non-useful records.
"""
...
try:
    ...
    HAS_GEMINI = False
    print("Note: google-genai not installed. LLM features disabled.", file=sys.stderr)

class LLMExtractor:
    """Extracts company information using Google Gemini LLM via google-genai SDK"""
    ...
    def _get_best_model(self) -> Optional[str]:
        ...
        try:
            ...
            # Filter for Gemini models by name (simplest reliable method)
            # The SDK might return 'models/gemini-1.5-flash' or just 'gemini-1.5-flash'
            supported_models = [
            ...
            # Check for exact matches or matches ending with the preference
            for pref in preferences:
                for model in supported_models:
                    if model == pref or model.endswith(f"/{pref}"):
                        print(f"Using LLM Model: {model}", file=sys.stderr)
                        return model
            
            # Fallback: take the first available if no preference matches
            if supported_models:
                print(f"Using fallback LLM Model: {supported_models[0]}", file=sys.stderr)
                return supported_models[0]
                
        except Exception as e:
            print(f"Error listing models: {e}", file=sys.stderr)
            # Hard fallback if list fails
            print("Using hard fallback: gemini-pro", file=sys.stderr)
            return "gemini-pro"
            
        return None
        
    def extract(self, text: str, url: str) -> Dict[str, Optional[str]]:
        """
        Extract company info from text using LLM
        """
        ...
        prompt = f"""
        Analyze the following text from the website {url} and extract the company details.
        ...
        Do not make up information. Use only what is present in the text.
        ...
        if not self.model_name:
             print("No suitable LLM model found.", file=sys.stderr)
             return {}
        ...
        try:
            ...
            required_keys = ['company_name', 'email', 'phone', 'address']
            ...
            print(f"LLM Extraction failed with model {self.model_name}: {e}", file=sys.stderr)
            return {}
...
class WebSearchAgent:
    ...
    def __init__(self, api_key: str, search_engine_id: str, gemini_key: Optional[str] = None):
        """
        ...
        Args:
            ...
            gemini_key: Google Gemini API Key (optional, for v4 reasoning)
        """
        ...
        if gemini_key and HAS_GEMINI:
            self.llm_extractor = LLMExtractor(gemini_key)
    ...
    def translate_text(self, text: str) -> str:
        ...
        # (Or just always try translating short strings like names/addresses if they aren't ASCII)
        try:
        ...
        try:
            ...
            if self.llm_extractor:
                try:
                    ...
                    llm_data = self.llm_extractor.extract(text, url)
                    if llm_data:
                        if llm_data.get('company_name'):
                            info['company_name'] = llm_data['company_name']
                        if llm_data.get('email'):
                            info['email'] = llm_data['email']
                        if llm_data.get('phone'):
                            info['phone'] = llm_data['phone']
                        if llm_data.get('address'):
                            info['address'] = llm_data['address']
                except Exception as e:
                    print(f"LLM extraction error for {url}: {e}", file=sys.stderr)

            # --- Fallback/Supplement with Regex/Soup if LLM missing or failed ---
    ...
    def is_valid_record(self, info: Dict[str, Any]) -> bool:
        """
        Check if a record is valid (not empty/null/non-useful and is a company site)
        ...
        # Helper to clean and check a value
        def is_meaningful(value):
            ...
            v = value.strip().lower()
            if v in ['', 'none', 'null', 'n/a', 'undefined', 'unknown']:
                return False
            return True

        # Check essential fields
        has_email = is_meaningful(info.get('email'))
        has_phone = is_meaningful(info.get('phone'))
        has_address = is_meaningful(info.get('address'))
        
        # Keep if it has at least one channel of contact
        return has_email or has_phone or has_address

    def filter_records(self, records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Filter out records that are empty or non-useful
...
def main():
    ...
    # Prompt for Gemini Key if we want reasoning features
    if not gemini_key:
        print("\n[Optional] Enter Google Gemini API Key for enhanced reasoning (Address/Phone extraction).")
        print("Press Enter to skip (regular regex extraction only).")
        gemini_input = input("Gemini API Key: ").strip()
        if gemini_input:
            gemini_key = gemini_input
        elif api_key: 
            # Try reusing the Search API key if user didn't provide specific one, 
            # often they are the same project
            gemini_key = api_key
    ...
    agent = WebSearchAgent(api_key, search_engine_id, gemini_key)
